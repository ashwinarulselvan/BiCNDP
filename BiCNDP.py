...
def biCNDP(args):
    ...
    inp = {'numnodes':100, 'gtype':"er", 'p':0.3, 
           ...
           'dbug':True, 'test':True, 'ccut':True, 'os':0, 'file':0}

    '''for key,value in kwargs.items():
            if key == 'numnodes' or key == 'test' or key == 'dbug' or key=='ccut':value = int(value)
            elif key=='os': value = value
            else: value = float(value)
            inp[key] = value    
    '''

    file = open("parfile.txt", "r")

    for line in file:
        data = line.strip().split()
        #print (line, data)
        key =  data[0]
        if key == 'numnodes' or key == 'test' or key == 'dbug' or key=='ccut' or key=='file':value = int(data[1])
        elif key == 'os' or key == 'gtype': value = data[1] 
        else: value = float(data[1])
        inp[key] = value
    file.close()

    if inp['file'] != 0 : inp['os'] = inp['os']+args
    ...
    I = genInput()
    if inp['file'] == 1: 
        I.initialise_data(numnodes=inp['numnodes'], gtype = inp['gtype'], edge_density = inp['edge_density'], 
                          ws_param =inp['ws_param'], itemcosts=inp['itemcosts'],
                          dbug = inp['dbug'], test=inp['test'],
                          ccut = inp['ccut'], os = inp['os'], file = inp['file'])
        I.write_file(inp['os']) 
        return
    if inp['file'] == 2: 
        I.read_file(numnodes=inp['numnodes'], gtype = inp['gtype'], edge_density = inp['edge_density'], 
                          ws_param =inp['ws_param'], itemcosts=inp['itemcosts'],
                          dbug = inp['dbug'], test=inp['test'],
                          ccut = inp['ccut'], os = inp['os'], file = inp['file'])
    else: I.initialise_data(numnodes=inp['numnodes'], test=inp['test'])
    n=I.N
    ...
    print ("objective:", obj, mip.get_mip_relative_gap(), curr)
    file = open("results.csv", "a")
    file.write(",".join(map(str,inp.values())))
    file.write(", {0}, {1}, {2},\n".format(obj, mip.get_mip_relative_gap(), curr))
